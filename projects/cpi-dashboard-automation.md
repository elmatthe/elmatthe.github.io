---
layout: page
title: CPI Dashboard Automation
permalink: /projects/cpi-dashboard-automation/
summary: CPI data download and spreadsheet workflow project with downloadable templates.
last_updated: 2026-03-13
---

# CPI Dashboard Automation

This project supports a practical inflation-monitoring workflow for research and advisory prep, combining Python automation with downloadable spreadsheet templates.

## Downloads
<div class="btn-row">
  <a class="btn" href="{{ '/assets/downloads/cpi_portfolio_rebalance_template.xlsx' | relative_url }}" download>Download CPI Template (.xlsx)</a>
  <a class="btn btn-secondary" href="{{ '/assets/downloads/cpi_portfolio_rebalance_template.xlsm' | relative_url }}" download>Download CPI Macro Template (.xlsm)</a>
</div>

## Project Files
- [CPI Dashboard Downloader (Python v7)](/Projects/CPI_Automation/cpi_dashboard_downloader-v7.py)
- [CPI Downloader User Guide](/Projects/CPI_Automation/CPI_Downloader_User_Guide.md)

## How this connects to your advisory workflow
1. Automate monthly CPI data pulls with Python.
2. Store and organize CPI series in a spreadsheet model.
3. Use the output in planning discussions (inflation assumptions, cash-flow stress tests, and portfolio review commentary).
