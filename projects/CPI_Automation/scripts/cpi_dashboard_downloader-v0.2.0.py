...
# Earliest month with data across all sources (FRED Headline CPI CPIAUCNS).
MIN_YM = "1913-01"
...
# scripts/<this file> -> repo root -> files/config.json
CONFIG_PATH = Path(__file__).resolve().parent.parent / "files" / "config.json"

def load_api_key() -> str:
    """Return the saved FRED API key, or '' if the config is missing or corrupted."""
    try:
        data = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
        return data.get("fred_api_key") or ""
    except Exception:
        return ""

def save_api_key(api_key: str):
    """Save the FRED API key for next launch. Never raises on failure."""
    try:
        CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
        CONFIG_PATH.write_text(json.dumps({"fred_api_key": api_key}), encoding="utf-8")
    except Exception:
        pass
...
def build_rows(start_ym: str, end_ym: str, api_key: str):
    ...
    # Drop leading rows with no data from any source (e.g. months before
    # the earliest available data point). Once a row with real data shows
    # up, keep everything from there on as-is, including the end-date row.
    first_data_idx = None
    for i, row in enumerate(rows):
        if any(row[col] is not None for col in OUTPUT_COLUMNS if col != "Date"):
            first_data_idx = i
            break

    if first_data_idx is not None:
        rows = rows[first_data_idx:]
...
def run_download():
    ...
    try:
        ...
        # Data availability begins MIN_YM across all sources; clamp quietly
        # rather than erroring (single mode keeps start/end in sync).
        if start_ym < MIN_YM:
            start_ym = MIN_YM
            if mode == "single":
                end_ym = start_ym
...
def main():
    ...
    entry_apikey.insert(0, load_api_key())
