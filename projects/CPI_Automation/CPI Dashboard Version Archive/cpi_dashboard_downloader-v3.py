...
# ========= STATCAN FETCH via range (NEW: fixes long-range timeout) =========
def fetch_statcan_vector_range(vector_id: int, start_ym: str, end_ym: str, max_retries=3):
    """
    Use getDataFromVectorByReferencePeriodRange (GET, supports full ranges).
    Returns dict {'YYYY-MM': value}.
    """
    start_date = f"{start_ym}-01"
    end_date = f"{end_ym}-01"  # API uses refPer start of month
    ...
    for attempt in range(max_retries):
        try:
            ...
            j = r.json()
            data = {}
            # Parse status/object structure (similar to latestN)
            if isinstance(j, list) and len(j) > 0 and j[0].get("status") == "SUCCESS":
                obj = j[0].get("object", {})
...
def fetch_all_statcan(start_ym: str, end_ym: str):
    ...
    for col, vec_id in STATCAN_VECTORS.items():
        out[col] = fetch_statcan_vector_range(vec_id, start_ym, end_ym)
    return out

# ========= FRED FETCH (US) – UNCHANGED, but with longer timeout + retry =========
def fetch_fred_monthly(series_id: str, start_ym: str, end_ym: str, max_retries=3):
    params = {
        "series_id": series_id,
...
def fetch_fred_daily_eom(series_id: str, start_ym: str, end_ym: str, max_retries=3):
...
def main():
    ...
    root = tk.Tk()
    root.title("CPI Dashboard Downloader v3 (Canada + US) - Long Range Fixed")
