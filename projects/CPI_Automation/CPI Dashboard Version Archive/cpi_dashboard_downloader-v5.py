...
def build_rows(start_ym: str, end_ym: str):
    ...
    for ym in month_range(start_ym, end_ym):
        ...
        # Date as a real Python date object — Excel will store it as a date serial
...
def write_to_xlsm(rows, xlsm_path: str):
    """
    Open the target .xlsm workbook (preserving VBA), clear the CPI_Downloader
    sheet, write headers + data, then save in place.

    The Date column is written as a Python date object so Excel treats it as
    a proper date — format the column as 'mmmm yyyy' (or any date format) in
    Excel and it will display correctly.
    """
