---
layout: page
title: Monte Carlo Setup Guide
permalink: /projects/monte-carlo-guide/
summary: Setup and usage guide for the Monte Carlo Retirement Simulator project.
last_updated: 2026-03-19
---

<section class="callout">
  <p><strong>Guide access:</strong> this is the rendered web version of the Monte Carlo setup guide.</p>
  <p><a href="{{ '/projects/Monte_Carlo_Simulator/Monte_Carlo_Setup_Guide.md' | relative_url }}" download>Download raw guide (.md)</a></p>
  <p><a href="{{ '/projects/Monte_Carlo_Simulator/monte_carlo_simulator.py' | relative_url }}" download>Download desktop starter script (.py)</a></p>
</section>

{% include_relative Monte_Carlo_Simulator/Monte_Carlo_Setup_Guide.md %}
